...
router.post('/spin', async (req, res) => {
  try {
    ...
    // Validate roulette_id (should be 2 for domino)
    if (!roulette_id) {
    ...
    console.log(`🎯 Win probability: ${(domino_probability * 100).toFixed(2)}%`);

    // Check if wins according to probability
    if (random > domino_probability) {
      // Didn't win
      return res.json({
        success: true,
        winner: false,
    ...
    // Won! Search for available prizes (with available stock) for domino
    ...
    console.error('Error in domino spin:', error);
    res.status(500).json({
      success: false,
      message: 'Error al lanzar el dominó',
      error: error.message
    });
  }
});

// POST /api/domino/winner - Register winner or participant
...
// GET /api/domino/stats - Get domino statistics
router.get('/stats', async (req, res) => {
  try {
    // Filter prizes for domino (ID_RULETA: 2)
    const prizes = await Stock.find({ ID_RULETA: 2 }).sort({ ID_PREMIO: 1 });
    ...
    res.json({
      success: true,
      winProbability: `${(domino_probability * 100)}%`,
      totalWinners,
      prizes: stats
    });
...
// GET /api/domino/winners/download - Download domino winners as Excel
router.get('/winners/download', async (req, res) => {
  try {
    // Fetch all domino winners from database (ID_RULETA: 2), sorted by creation date (newest first)
    const winners = await Winner.find({ roulette_id: 2 }).sort({ createdAt: -1 });
    ...
    XLSX.utils.book_append_sheet(workbook, worksheet, 'Ganadores Domino');
    ...
    const now = new Date();
    const filename = `ganadores_domino_${now.getFullYear()}${String(now.getMonth() + 1).padStart(2, '0')}${String(now.getDate()).padStart(2, '0')}_${String(now.getHours()).padStart(2, '0')}${String(now.getMinutes()).padStart(2, '0')}.xlsx`;

    // Set headers for file download
    res.setHeader('Content-Disposition', `attachment; filename="${filename}"`);
    res.setHeader('Content-Type', 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet');

    console.log(`📥 Downloaded ${winners.length} domino winners as Excel: ${filename}`);
