...
const { cyber_probability } = require('../config/roulette');

// POST /api/cyber/spin - Simulate cyber roulette spin
router.post('/spin', async (req, res) => {
  try {
    const { roulette_id } = req.body;

    // Validate roulette_id (should be 3 for cyber)
    if (!roulette_id) {
    ...
    console.log(`🎲 Generated probability: ${(random * 100).toFixed(4)}%`);
    console.log(`🎯 Win probability: ${(cyber_probability * 100).toFixed(4)}%`);

    // Check if wins according to probability
    if (random > cyber_probability) {
      // Didn't win
      return res.json({
        success: true,
        winner: false,
    ...
    // Won! Search for available prizes (with available stock) for cyber
    ...
    res.json({
      success: true,
      winner: true,
      message: wonPrize.PREMIO, // Return prize name as message
      prize: {
    ...
    console.error('Error in cyber spin:', error);
    res.status(500).json({
      success: false,
      message: 'Error al lanzar la ruleta cyber',
      error: error.message
    });
  }
});

// POST /api/cyber/winner - Register winner or participant
...
// GET /api/cyber/stats - Get cyber statistics
router.get('/stats', async (req, res) => {
  try {
    // Filter prizes for cyber (ID_RULETA: 3)
    ...
    res.json({
      success: true,
      winProbability: `${(cyber_probability * 100).toFixed(4)}%`,
      totalWinners,
      totalPrizes: totalStock,
      estimatedPlays,
      prizes: stats
    });
...
// GET /api/cyber/winners/download - Download cyber winners as Excel
router.get('/winners/download', async (req, res) => {
  try {
    // Fetch all cyber winners from database (ID_RULETA: 3), sorted by creation date (newest first)
    ...
    XLSX.utils.book_append_sheet(workbook, worksheet, 'Ganadores Cyber');
    ...
    const now = new Date();
    const filename = `ganadores_cyber_${now.getFullYear()}${String(now.getMonth() + 1).padStart(2, '0')}${String(now.getDate()).padStart(2, '0')}_${String(now.getHours()).padStart(2, '0')}${String(now.getMinutes()).padStart(2, '0')}.xlsx`;

    // Set headers for file download
    res.setHeader('Content-Disposition', `attachment; filename="${filename}"`);
    res.setHeader('Content-Type', 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet');

    console.log(`📥 Downloaded ${winners.length} cyber winners as Excel: ${filename}`);
