...
router.post('/spin', async (req, res) => {
  try {
    ...
    // Validate roulette_id (should be 6 for Milex)
    if (!roulette_id) {
    ...
    console.log(`🎯 Win probability: ${(milex_probability * 100).toFixed(2)}%`);

    // Check if wins according to probability
    if (random > milex_probability) {
      // Didn't win
      return res.json({
        success: true,
        winner: false,
    ...
    // Won! Search for available prizes (with available stock) for Milex
    ...
    console.error('Error in Milex spin:', error);
    res.status(500).json({
      success: false,
...
// POST /api/milex/winner - Register winner or participant
...
// GET /api/milex/stats - Get Milex roulette statistics
router.get('/stats', async (req, res) => {
  try {
    // Filter prizes for Milex (ID_RULETA: 6)
    const prizes = await Stock.find({ ID_RULETA: 6 }).sort({ ID_PREMIO: 1 });
    ...
    const stats = prizes.map(prize => ({
      ...
      percentage: ((prize.Stock - prize.GANADORES) / prize.Stock * 100).toFixed(2) + '%'
    }));

    const totalWinners = prizes.reduce((sum, p) => sum + p.GANADORES, 0);
    const totalStock = prizes.reduce((sum, p) => sum + p.Stock, 0);
    const totalAvailable = prizes.reduce((sum, p) => sum + (p.Stock - p.GANADORES), 0);

    res.json({
      success: true,
      rouletteName: 'Milex',
      rouletteId: 6,
      winProbability: `${(milex_probability * 100)}%`,
      totalWinners,
      totalStock,
      totalAvailable,
      availabilityPercentage: ((totalAvailable / totalStock) * 100).toFixed(2) + '%',
      prizes: stats
    });
...
// GET /api/milex/winners/download - Download Milex winners as Excel
router.get('/winners/download', async (req, res) => {
  try {
    // Fetch all Milex winners from database (ID_RULETA: 6), sorted by creation date (newest first)
    const winners = await Winner.find({ roulette_id: 6 }).sort({ createdAt: -1 });
    ...
    XLSX.utils.book_append_sheet(workbook, worksheet, 'Ganadores Milex');
    ...
    const now = new Date();
    const filename = `ganadores_milex_${now.getFullYear()}${String(now.getMonth() + 1).padStart(2, '0')}${String(now.getDate()).padStart(2, '0')}_${String(now.getHours()).padStart(2, '0')}${String(now.getMinutes()).padStart(2, '0')}.xlsx`;

    // Set headers for file download
    res.setHeader('Content-Disposition', `attachment; filename="${filename}"`);
    res.setHeader('Content-Type', 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet');

    console.log(`📥 Downloaded ${winners.length} Milex winners as Excel: ${filename}`);
