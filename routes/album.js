...
const { album_probability } = require('../config/roulette');

// POST /api/album/spin - Simulate album game spin
router.post('/spin', async (req, res) => {
  try {
    const { roulette_id } = req.body;

    // Validate roulette_id (should be 3 for album)
    if (!roulette_id) {
    ...
    console.log(`Win probability: ${(album_probability * 100).toFixed(2)}%`);

    // Check if wins according to probability (with 100% probability, this should always win)
    if (random > album_probability) {
      // Didn't win (this should rarely/never happen with 100% probability)
      return res.json({
        success: true,
        winner: false,
    ...
    // Won! Search for available prizes (with available stock) for album
    ...
    console.error('Error in album spin:', error);
    res.status(500).json({
      success: false,
      message: 'Error al lanzar el álbum',
      error: error.message
    });
  }
});

// POST /api/album/winner - Register winner or participant
...
// GET /api/album/stats - Get album statistics
router.get('/stats', async (req, res) => {
  try {
    // Filter prizes for album (ID_RULETA: 3)
    ...
    res.json({
      success: true,
      winProbability: `${(album_probability * 100)}%`,
      totalWinners,
      prizes: stats
    });
...
// GET /api/album/winners/download - Download album winners as Excel
router.get('/winners/download', async (req, res) => {
  try {
    // Fetch all album winners from database (ID_RULETA: 3), sorted by creation date (newest first)
    ...
    XLSX.utils.book_append_sheet(workbook, worksheet, 'Ganadores Álbum');
    ...
    const now = new Date();
    const filename = `ganadores_album_${now.getFullYear()}${String(now.getMonth() + 1).padStart(2, '0')}${String(now.getDate()).padStart(2, '0')}_${String(now.getHours()).padStart(2, '0')}${String(now.getMinutes()).padStart(2, '0')}.xlsx`;

    // Set headers for file download
    res.setHeader('Content-Disposition', `attachment; filename="${filename}"`);
    res.setHeader('Content-Type', 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet');

    console.log(`Downloaded ${winners.length} album winners as Excel: ${filename}`);
