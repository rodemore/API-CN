...
const { py_probability } = require('../config/roulette');

// POST /api/ruletapy/spin - Simulate RuletaPY game spin
router.post('/spin', async (req, res) => {
  try {
    const { roulette_id } = req.body;

    // Validate roulette_id (should be 5 for RuletaPY)
    if (!roulette_id) {
    ...
    console.log(`🎯 Win probability: ${(py_probability * 100).toFixed(2)}%`);

    // Check if wins according to probability
    if (random > py_probability) {
      // Didn't win
      return res.json({
        success: true,
        winner: false,
    ...
    // Won! Search for available prizes (with available stock) for RuletaPY
    ...
    console.error('Error in RuletaPY spin:', error);
    res.status(500).json({
      success: false,
      message: 'Error al lanzar la RuletaPY',
      error: error.message
    });
  }
});

// POST /api/ruletapy/winner - Register winner or participant
...
// GET /api/ruletapy/stats - Get RuletaPY statistics
router.get('/stats', async (req, res) => {
  try {
    // Filter prizes for RuletaPY (ID_RULETA: 5)
    const prizes = await Stock.find({ ID_RULETA: 5 }).sort({ ID_PREMIO: 1 });
    ...
    res.json({
      success: true,
      winProbability: `${(py_probability * 100)}%`,
      totalWinners,
      prizes: stats
    });
...
// GET /api/ruletapy/winners/download - Download RuletaPY winners as Excel
router.get('/winners/download', async (req, res) => {
  try {
    // Fetch all RuletaPY winners from database (ID_RULETA: 5), sorted by creation date (newest first)
    const winners = await Winner.find({ roulette_id: 5 }).sort({ createdAt: -1 });
    ...
    XLSX.utils.book_append_sheet(workbook, worksheet, 'Ganadores RuletaPY');
    ...
    const now = new Date();
    const filename = `ganadores_ruletapy_${now.getFullYear()}${String(now.getMonth() + 1).padStart(2, '0')}${String(now.getDate()).padStart(2, '0')}_${String(now.getHours()).padStart(2, '0')}${String(now.getMinutes()).padStart(2, '0')}.xlsx`;

    // Set headers for file download
    res.setHeader('Content-Disposition', `attachment; filename="${filename}"`);
    res.setHeader('Content-Type', 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet');

    console.log(`📥 Downloaded ${winners.length} RuletaPY winners as Excel: ${filename}`);
